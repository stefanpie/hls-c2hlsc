...
def compile_and_run_benchmark(
    ...
    print(f"Running Compile and Run : {benchmark_name} : {temp_dir_path}")
    ...
    makefile_txt += f"{benchmark_name}: {benchmark_name}_tb.cpp {benchmark_name}.cpp {benchmark_name}.h\n"
    makefile_txt += f"\t$(CC) $(CFLAGS) -o {benchmark_name} {benchmark_name}_tb.cpp {benchmark_name}.cpp\n"
...
def main(args):
    ...
    benchmarks_to_test = [
        benchmark for benchmark in benchmarks_to_test if len(list(benchmark.glob("*")))
    ]
    ...
    # )
    return_data_compile_run = [
        compile_and_run_benchmark(benchmark, temp_dir_overide=temp_dir_compile)
        for benchmark in benchmarks_to_test
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "benchmarks_directory", type=Path, nargs="?", default=Path("./hls-c2hlsc/")
    )
